...
object ServiceModule {
    ...
    fun provideMainActivityPendingIntent(@ApplicationContext app : Context) {PendingIntent.getActivity(
        app ,
        0,
        Intent().also {it.action = Constants.ACTION_SHOW_TRACKING_FRAGMENT },
    ...
    @ServiceScoped
    @Provides
    fun providesBaseNotificationBuilder(@ApplicationContext app : Context , pendingIntent: PendingIntent) = NotificationCompat.Builder(app, Constants.NOTIFICATION_CHANNEL_ID)
        .setAutoCancel(false)
        .setOngoing(true)
